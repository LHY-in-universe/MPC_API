...
impl BFVBeaverGenerator {
    ...
    /// BFV同态加法
    fn homomorphic_add(&self, ct1: &BFVCiphertext, ct2: &BFVCiphertext) -> Result<BFVCiphertext> {
        if ct1.c0.len() != ct2.c0.len() || ct1.c1.len() != ct2.c1.len() {
    ...
    /// BFV同态减法
    fn homomorphic_subtract(&self, ct1: &BFVCiphertext, ct2: &BFVCiphertext) -> Result<BFVCiphertext> {
        if ct1.c0.len() != ct2.c0.len() || ct1.c1.len() != ct2.c1.len() {
            return Err(MpcError::CryptographicError("Ciphertext dimensions mismatch".to_string()));
        }
