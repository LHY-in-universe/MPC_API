...
mod tests {
    ...
    #[test]
    fn test_free_xor_optimizer() {
        let mut rng = thread_rng();
        let global_offset = generate_random_label(&mut rng);
        let optimizer = FreeXorOptimizer::new(global_offset);
        
        // Test wire label generation
        let wire_labels = optimizer.generate_wire_labels(5);
        assert_eq!(wire_labels.len(), 5);
        assert!(optimizer.verify_free_xor_property(&wire_labels));
    }
    
    #[test]
    fn test_free_xor_evaluation() {
        let mut rng = thread_rng();
        let global_offset = generate_random_label(&mut rng);
        let optimizer = FreeXorOptimizer::new(global_offset);
        
        let label1 = generate_random_label(&mut rng);
        let label2 = generate_random_label(&mut rng);
        
        let result = optimizer.evaluate_free_xor(&label1, &label2);
        let expected = xor_labels(&label1, &label2);
    ...
    fn test_gate_classification() {
        ...
        let xor_gate = Gate::new(0, GateType::Xor, vec![0, 1], 2);
        let and_gate = Gate::new(1, GateType::And, vec![0, 1], 2);
        let not_gate = Gate::new(2, GateType::Not, vec![0], 1);
        
        assert!(optimizer.can_use_free_xor(&xor_gate));
        assert!(!optimizer.can_use_free_xor(&and_gate));
        assert!(optimizer.can_use_free_xor(&not_gate));
    }
    
    #[test]
    fn test_xor_heavy_circuit() {
        let circuit = create_xor_heavy_circuit(4);
        
        // Should have 8 input wires (4 * 2)
        assert_eq!(circuit.input_wires.len(), 8);
        
        // Should have 1 output wire
        assert_eq!(circuit.output_wires.len(), 1);
        
        // Should be mostly XOR gates
        assert!(is_linear_circuit(&circuit));
        assert_eq!(count_non_linear_gates(&circuit), 0);
    }
    
    #[test]
    fn test_communication_savings() {
        let mut rng = thread_rng();
        let global_offset = generate_random_label(&mut rng);
        let optimizer = FreeXorOptimizer::new(global_offset);
        
        let xor_circuit = create_xor_heavy_circuit(4);
        let (without_opt, with_opt) = optimizer.calculate_savings(&xor_circuit);
        
        // XOR-heavy circuit should have significant savings
        assert!(with_opt < without_opt);
        
        // In fact, for a pure XOR circuit, communication should be 0
        assert_eq!(with_opt, 0);
    }
}